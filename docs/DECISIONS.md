...
Numbered decision log for this project — settled architectural calls, in chronological order. Each entry records what was decided, when, and (where relevant) the *why* behind it. Treat these as binding unless explicitly revisited with the user.
...
51. **`phpinfo()` output for our four own extensions (`navicat`, `jsonk`,
    `mdhtml`, `norm`) redesigned as a footer, not a header, and `navicat`
    was missing its own `version` row (2026-09-17).** Requested by the
    user after seeing PHP core's own Zend Engine credit box, which sits at
    the bottom of a module's info table rather than the top — the
    Kirigami logo previously sat in a centered `colspan="2"` row right
    after `php_info_print_table_start()`; it now sits as the last row,
    right before `php_info_print_table_end()`, as a two-cell row (small
    left-aligned text — `"Part of the Kirigami PHP extension family"` —
    plus the same hosted `kirigami-logo-universal.svg` right-aligned).
    Same edit applied identically across all four extensions' own repos,
    each with its own version bump and tag:
    - `navicat`: `v0.1.3` → `v0.1.4` (footer move) → `v0.1.5` (added the
      missing `php_info_print_table_row(2, "version",
      PHP_NAVICAT_VERSION)` row — the other three already had one, this
      one didn't).
    - `jsonk`: `v0.1.3` → `v0.1.4` (footer move only; already had its
      `version` row).
    - `mdhtml`: `v0.1.1` → `v0.1.2` (footer move only; already had its
      `version` row).
    - `norm`: `v0.1.0` → `v0.1.1` (footer move only; already had its
      `version` row).
    - `matrix.json` bumped to match (`extensions.navicat.versions` gained
      `v0.1.4`/`v0.1.5`, `extensions.jsonk.versions` gained `v0.1.4`,
      `extensions.mdhtml.versions` gained `v0.1.2`,
      `extensions.norm.versions` gained `v0.1.1`) — no `config.yaml` or
      `compile/php/Dockerfile` change needed, same "`getMatrixExtensionVersion()`
      resolves the last `versions[]` entry automatically" mechanism as
      decision 50.
    - **✅ Real build run same day, exit code 0** (`node compile/cli.mjs
      --quiet`), producing `node-builds/8-5/php_8_5.js` +
      `8_5_10/php_8_5.wasm` (26,957,515 bytes) — this also picks up
