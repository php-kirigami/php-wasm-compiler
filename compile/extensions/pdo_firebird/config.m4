...
if test "$PHP_PDO_FIREBIRD" != "no"; then
  ...
  if test -x "$FB_CONFIG" && test "$PHP_PDO_FIREBIRD" = "yes"; then
    ...
    PHP_CHECK_LIBRARY([fbclient], [fb_get_master_interface],
      ...
      [$FIREBIRD_LIBDIR_FLAG])
    dnl PHP_ADD_LIBRARY_WITH_PATH([fbclient],
    dnl [$FIREBIRD_LIBDIR],
    dnl [PDO_FIREBIRD_SHARED_LIBADD]) is deliberately removed here (not the
    dnl real upstream config.m4) -- same libtool failure tidy/bz2/pdo_dblib
    dnl hit: a bare "-L... -lfbclient" against a static-only archive with no
    dnl .la sidecar silently downgrades the build to a static-only module.
    dnl config.yaml's vendorLib mechanism already passes the real archives
    dnl to the final link; see compile/extensions/pdo_firebird/PROVENANCE.md.
