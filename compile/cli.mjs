...
/**
 * Generates the default-exported `register(phpVersion)` entry point every
 * @kirigami/phpext-* package ships (CLAUDE.md decision 42, per the user's
...
/**
 * Hash of the actual compiled output (every manifest*.json + *.so, name and
 ...
 * nothing"). Deterministic: file names are sorted before hashing. Matches
 * both a normal single-manifest package and a bundled one (decision 42 —
 * e.g. mysqli's package also carries manifest-mysqlnd.json).
 */
function computeBuildHash(outDir) {
	const files = readdirSync(outDir)
		.filter((f) => (f.startsWith('manifest') && f.endsWith('.json')) || f.endsWith('.so'))
		.sort();
 ...
 * matrix.json-pinned version when the extension has one, the load order for
 * any other extension bundled into the same package (decision 42 — e.g.
 * mysqlnd bundled into mysqli's own package, `manifest-mysqlnd.json` must be
 * loaded before this package's own `manifest.json`), and the same
...
function buildKirigamiExtensionMetadata(ext, phpVersionList, buildHash) {
	...
	if (ext.bundleExtensions?.length) {
		...
		// loaded before this package's own manifest.json.
		kirigami.bundles = ext.bundleExtensions;
	}
...
function syncExtensionPackage(outDir, ext, phpVersionList) {
	...
	writeFileSync(path.join(outDir, 'index.d.ts'), extensionIndexDts(), 'utf8');
...
/**
 * `--extra-cflags`/`--extra-ldflags`/`--config-args` for one extension's
 * compile-extension invocation (vendorLib + pkgConfigVar resolution) — pure
 * string computation, shared between a normal build and a bundled one
 * (CLAUDE.md decision 42's mysqlnd+mysqli pilot).
 */
function buildExtensionArgs(ext, phpVersions, outDir) {
	...
	return { extSourceDir, args };
}

/**
 * Runs one extension's compile-extension invocation into `outDir`, renaming
 * the tool's always-`manifest.json` output to `manifestName` afterward. Used
 * both for an extension's own build (manifestName: 'manifest.json') and for
 * bundling another extension's artifact into the same package directory
 * (CLAUDE.md decision 42 — e.g. mysqlnd bundled into @kirigami/phpext-mysqli,
 * since mysqli's config.m4 declares PHP_ADD_EXTENSION_DEP(mysqli, mysqlnd)
 * and its .so genuinely needs mysqlnd's Zend module loaded first, not just a
 * linkable C library).
 */
...
async function runCompileExtensionCommand(argv) {
	...
	// Extensions referenced by another's `bundleExtensions` (e.g. mysqlnd,
	// bundled into mysqli's own package) are only built as part of that
	// bundle in a full run — building them again on their own would just be
	// redundant Docker time. Still directly targetable by name for isolated
	// debugging (`compile-extension mysqlnd`).
	const bundledNames = new Set(shared.flatMap((ext) => ext.bundleExtensions ?? []));
	const selected = argv.name
		? shared.filter((ext) => ext.name === argv.name)
		: shared.filter((ext) => !bundledNames.has(ext.name));
	...
	for (const ext of selected) {
		...
		const bundled = (ext.bundleExtensions ?? []).map((name) => {
			const bundledExt = shared.find((e) => e.name === name);
			if (!bundledExt) {
				throw new Error(
					`config.yaml: "${ext.name}" bundles "${name}", but there's no ` +
						`mode:shared extension named "${name}".`
				);
			}
			return bundledExt;
		});
		...
		if (argv['dry-run']) {
			for (const bundledExt of bundled) {
				const { args } = buildExtensionArgs(bundledExt, phpVersions, outDir);
				console.log(`\n[dry-run] ${compileExtensionBin} ${args.join(' ')}`);
				console.log(`[dry-run] rename manifest.json -> manifest-${bundledExt.name}.json`);
			}
			const { args } = buildExtensionArgs(ext, phpVersions, outDir);
			console.log(`\n[dry-run] ${compileExtensionBin} ${args.join(' ')}`);
			if (!isInternal) {
				console.log(`[dry-run] write ${path.join(outDir, 'package.json')}, index.js, and index.d.ts`);
			}
			continue;
		}

		for (const bundledExt of bundled) {
			await buildOneExtensionArtifact(
				bundledExt,
				outDir,
				phpVersions,
				compileExtensionBin,
				`manifest-${bundledExt.name}.json`
			);
		}
		await buildOneExtensionArtifact(ext, outDir, phpVersions, compileExtensionBin, 'manifest.json');
...
function buildArgsForVersion(config, phpVersion) {
	...
	// php-kirigami/php-jsonk — our own extension, hard-depends on ext/json
	// (which can't be disabled, see config.yaml) and vendors simdjson +
	// yyjson as flat files rather than a separate compile/lib* target.
	args.push(`--JSONK_EXT_VERSION=${getMatrixExtensionVersion('jsonk')}`);
	args.push(`--SIMDJSON_VERSION=${getMatrixVersion('simdjson')}`);
	args.push(`--YYJSON_VERSION=${getMatrixVersion('yyjson')}`);
	// krakjoe/apcu — experiment (matrix.json's own "apcu" note).
	args.push(`--APCU_EXT_VERSION=${getMatrixExtensionVersion('apcu')}`);
	...
	// substitution this pipeline's plain git clone never runs).
	args.push(`--IMAGICK_EXT_VERSION=${getMatrixExtensionVersion('imagick')}`);
