...
# @kirigami/phpext-soap

`SoapClient`/`SoapServer` for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-soap)](https://www.npmjs.com/package/@kirigami/phpext-soap)
...
`@kirigami/phpext-soap` is PHP's own `ext/soap` — unmodified upstream
...
- ✅ Real `ext/soap`: the `SoapClient` and `SoapServer` classes, WSDL
  parsing, the full documented PHP API — not a reimplementation
- ✅ Linked against the same vendored, Emscripten-cross-compiled **libxml2**
  the core `php.wasm` build itself already uses
...
- [@kirigami/phpext-soap](#kirigamiphpext-soap)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-soap
```
...
| | |
| --- | --- |
| PHP extension | `ext/soap`, unmodified, from the `PHP-8.5.10` tag |
| PHP version(s) shipped | 8.5 |
| Native library | [libxml2](https://gitlab.gnome.org/GNOME/libxml2) (shared with `@kirigami/php-wasm`'s own core build) |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/soap`, is under the PHP License 3.01.
