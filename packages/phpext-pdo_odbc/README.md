...
# @kirigami/phpext-pdo_odbc

PDO `odbc:` driver for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-pdo_odbc)](https://www.npmjs.com/package/@kirigami/phpext-pdo_odbc)
...
`@kirigami/phpext-pdo_odbc` is PHP's own `ext/pdo_odbc` — unmodified upstream
...
- ✅ Real `ext/pdo_odbc`: the `odbc` PDO driver (`new PDO('odbc:...')`) — not a
  reimplementation
- ✅ Linked against the vendored, Emscripten-cross-compiled **unixODBC 2.3.14**
  driver manager (`libodbc`); independent of `@kirigami/phpext-odbc`
...
- [@kirigami/phpext-pdo_odbc](#kirigamiphpext-pdo_odbc)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-pdo_odbc
```
...
$pdo = new PDO('odbc:MyDSN', 'user', 'password');
...
| | |
| --- | --- |
| PHP extension | `ext/pdo_odbc`, unmodified, from the `PHP-8.5.11` tag |
...
| Patches | None — `ext/pdo_odbc` builds unmodified with `--with-pdo-odbc=unixODBC` |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/pdo_odbc`, is under the PHP License 3.01.
