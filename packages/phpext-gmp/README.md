...
Arbitrary-precision math (`gmp_*()`, the `GMP` class) for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-gmp)](https://www.npmjs.com/package/@kirigami/phpext-gmp)
...
`@kirigami/phpext-gmp` is PHP's own `ext/gmp` — unmodified upstream source,
...
- ✅ Real `ext/gmp`: `gmp_add`, `gmp_mul`, `gmp_pow`, the `GMP` object API,
  the full documented PHP API — not a reimplementation
- ✅ Linked against a vendored, Emscripten-cross-compiled **GNU MP 6.3.0**
  (portable generic-C code path — no per-CPU hand-written assembly, which
  GMP's own build gracefully falls back to for an unrecognized target like
  wasm32)
...
- [@kirigami/phpext-gmp](#kirigamiphpext-gmp)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-gmp
```
...
| | |
| --- | --- |
| PHP extension | `ext/gmp`, unmodified, from the `PHP-8.5.11` tag |
| PHP version(s) shipped | 8.5 |
| Native library | [GNU MP](https://gmplib.org) `6.3.0` |
| Build mode | `mode: shared` (JSPI side module) — see `@kirigami/php-wasm`'s `mode: static` core for the always-on extension set instead |
| Patches | None — `ext/gmp`'s own PHP-facing API needed no changes |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/gmp`, is under the PHP License 3.01.
