...
# @kirigami/phpext-intl

Internationalization (ICU: `NumberFormatter`, `IntlDateFormatter`, `Collator`, ...) for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-intl)](https://www.npmjs.com/package/@kirigami/phpext-intl)
...
`@kirigami/phpext-intl` is PHP's own `ext/intl`, upstream source straight from the `PHP-8.5.11` tag, compiled independently of the
...
- ✅ Real `ext/intl`: formatting, collation, normalization, transliteration, IDNA, break iteration, `IntlChar`, ...
- ✅ Linked against the vendored, Emscripten-cross-compiled **ICU 78.3**, with the **full ICU data compiled in** (about 32 MB; the module is about 40 MB)
- ✅ Takes over `Normalizer` from the core's php-norm fallback when installed
...
- [@kirigami/phpext-intl](#kirigamiphpext-intl)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-intl
```
...
| | |
| --- | --- |
| PHP extension | `ext/intl` from the `PHP-8.5.11` tag (unmodified) |
| Native library | [ICU](https://icu.unicode.org) `78.3` (libicuuc, libicui18n, libicuio, full data) |
| Patches | None |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/intl`, is under the PHP License 3.01. The statically linked ICU and its data are under the Unicode License v3.
