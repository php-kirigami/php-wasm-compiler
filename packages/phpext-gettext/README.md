...
# @kirigami/phpext-gettext

`gettext()`/`ngettext()`/... for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-gettext)](https://www.npmjs.com/package/@kirigami/phpext-gettext)
...
`@kirigami/phpext-gettext` is PHP's own `ext/gettext` — unmodified upstream
...
- ✅ Real `ext/gettext`: `gettext`, `ngettext`, `bindtextdomain`,
  `textdomain`, the full documented PHP API — not a reimplementation
- ✅ No external library needed — Emscripten's own musl-based libc already
  ships `<libintl.h>` and the underlying `gettext`/`ngettext`/`bindtextdomain`
  entry points this extension links against
- ⚠️ No real locale message-catalog lookup under this libc (musl's own
  `gettext` implementation is effectively a no-op passthrough) — the
  functions exist and return, but don't perform genuine `.mo` file
  translation the way glibc's would
...
- [@kirigami/phpext-gettext](#kirigamiphpext-gettext)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-gettext
```
...
| | |
| --- | --- |
| PHP extension | `ext/gettext`, unmodified, from the `PHP-8.5.11` tag |
| PHP version(s) shipped | 8.5 |
| Native library | None — Emscripten's own libc ships the `libintl.h` entry points this extension needs |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/gettext`, is under the PHP License 3.01.
