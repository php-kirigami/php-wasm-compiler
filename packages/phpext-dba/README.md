...
`dba_*()` for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-dba)](https://www.npmjs.com/package/@kirigami/phpext-dba)
...
`@kirigami/phpext-dba` is PHP's own `ext/dba` — unmodified upstream source,
straight from the `PHP-8.5.11` tag — compiled independently of the core
...
- ✅ Real `ext/dba`: `dba_open`, `dba_insert`, `dba_fetch`, `dba_close`, the
  full documented PHP API — not a reimplementation
- ✅ Three working backends, no external C library: `flatfile`, `inifile`,
  `cdb` (all bundled directly in php-src's own `ext/dba` sources) —
  every other backend (QDBM, GDBM, NDBM, Berkeley DB, Tokyo Cabinet, LMDB)
  needs an external library this build doesn't vendor and stays off
- ✅ JSPI only, Node.js only — matches `@kirigami/php-wasm`'s own target
- ✅ Verified end-to-end: a real `dba_open()`/`dba_insert()`/`dba_fetch()`
  round-trip against the `flatfile` backend
...
- [@kirigami/phpext-dba](#kirigamiphpext-dba)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-dba
```
...
```php
<?php
$db = dba_open('/tmp/example.db', 'n', 'flatfile');
dba_insert('greeting', 'hello', $db);
echo dba_fetch('greeting', $db);
dba_close($db);
```
...
| | |
| --- | --- |
| PHP extension | `ext/dba`, unmodified, from the `PHP-8.5.11` tag |
| PHP version(s) shipped | 8.5 |
| Native library | None — the three enabled backends (`flatfile`, `inifile`, `cdb`) all ship as bundled sources inside `ext/dba` itself |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/dba`, is under the PHP License 3.01.
