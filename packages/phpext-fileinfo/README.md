...
# @kirigami/phpext-fileinfo

File type detection (`finfo`, `mime_content_type()`) for Kirigami PHP.wasm — as a standalone, on-demand WASM side module.  
...
[![npm version](https://img.shields.io/npm/v/@kirigami/phpext-fileinfo)](https://www.npmjs.com/package/@kirigami/phpext-fileinfo)
...
`@kirigami/phpext-fileinfo` is PHP's own `ext/fileinfo`, with its bundled libmagic, upstream source straight from the `PHP-8.5.11` tag, compiled independently of the
...
- ✅ Real `ext/fileinfo`: `finfo`, `finfo_*()`, `mime_content_type()`, not a reimplementation
- ✅ The full magic database compiled in (about 10 MB of data, loaded with the module), so it works with no external `magic.mgc`
- ✅ JSPI only, Node.js only, matching `@kirigami/php-wasm`'s own target
...
- [@kirigami/phpext-fileinfo](#kirigamiphpext-fileinfo)
  - [Overview](#overview)
...
```bash
npm install @kirigami/phpext-fileinfo
```
...
echo $finfo->buffer(file_get_contents('photo.png')); // image/png
```
...
| | |
| --- | --- |
| PHP extension | `ext/fileinfo` from the `PHP-8.5.11` tag (unmodified) |
| Native library | php-src's bundled libmagic (no external library) |
| Patches | None |
...
`GPL-2.0-or-later`, the license of php-wasm-compiler's build pipeline, which comes from WordPress Playground (see its [NOTICE.md](https://github.com/php-kirigami/php-wasm-compiler/blob/main/NOTICE.md)). The extension's own source, php-src's `ext/fileinfo`, is under the PHP License 3.01. The bundled libmagic is under its own BSD-style license.
